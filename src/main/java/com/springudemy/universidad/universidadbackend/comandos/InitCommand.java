...
public class InitCommand implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Carrera ing = servicioCarrera.save(ObjetosDummy.getCarreraIngMec());
        Carrera lic = servicioCarrera.save(ObjetosDummy.getCarreraLicEco());
        ...
        Carrera ingInd = servicioCarrera.findById(ing.getId()).get();
        Carrera lisEco = servicioCarrera.findById(lic.getId()).get();
        ...
        ((Alumno)alumno1).setCarrera(ingInd);
        ((Alumno)alumno2).setCarrera(lisEco);
        ...
        Optional<Carrera> carreraOptional = servicioCarrera.findById(2);
        Iterable<Persona> all = servicioAlumno.findAll();

        all.forEach(alumno -> {
            if(alumno instanceof Alumno) {
                if(alumno.getId() > 2) {
                    ((Alumno)alumno).setCarrera(carreraOptional.get());
                }
            }
        });
